...
# Install bootmatch package
# library(remotes) # nolint
# remotes::install_github("tjmahr/bootmatch") # nolint

# load bootmatch package
library(bootmatch)
```
...
compare_matched_groups <- function(input_df, metrics_to_compare) {
  ...
  desc <- matrix(NA, nrow = length(unique(metrics_to_compare)), ncol = 1)
  desc <- as.data.frame(desc)
  colnames(desc) <- c("Score")
  desc$Score <- c(unique(metrics_to_compare))
